...
const API_URL = "https://serverless.roboflow.com/smartfirewatch_modelo_4-mdrcv/1";
const API_KEY = "H8ZX6BgARrnWxbwjHfhu";
const CONFIDENCE_THRESHOLD = 0.5;
const IMAGE_FOLDER = path.join(__dirname, "images");

const resultado = {
    modelo: "smartfirewatch_modelo_4-mdrcv/1",
    total_imagens: 0,
    ...
    confiancas: [],
    tempos_ms: [],
    detecções: [] // novo campo para armazenar informações detalhadas das detecções
};
...
fs.readdir(IMAGE_FOLDER, async (err, files) => {
    ...
    resultado.erro_medio = Number((1 - media).toFixed(4));
    ...
    fs.writeFileSync(
        path.join(__dirname, "modelo_4.json"),
        JSON.stringify(resultado, null, 4),
        "utf-8"
    );

    console.log("\n✅ Resultado salvo em modelo_4.json");
});
