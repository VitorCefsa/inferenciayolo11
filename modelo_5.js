...
const API_URL = "https://serverless.roboflow.com/smartfirewatch_modelo_5/1";
const API_KEY = "H8ZX6BgARrnWxbwjHfhu";
const CONFIDENCE_THRESHOLD = 0.5;
const IMAGE_FOLDER = path.join(__dirname, "images");

const resultado = {
    modelo: "smartfirewatch_modelo_5/1",
    total_imagens: 0,
    ...
    confiancas: [],
    tempos_ms: [],
    detecções: [] // novo campo para armazenar info das detecções
};
...
fs.readdir(IMAGE_FOLDER, async (err, files) => {
    ...
    fs.writeFileSync(
        path.join(__dirname, "modelo_5.json"),
        JSON.stringify(resultado, null, 4),
        "utf-8"
    );

    console.log("\n✅ Resultado salvo em modelo_5.json");
});
