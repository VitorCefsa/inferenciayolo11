...
const resultado = {
    ...
    confiancas: [],
    tempos_ms: [],
    detecções: [] // Novo campo para armazenar todas as detecções
};
...
fs.readdir(IMAGE_FOLDER, async (err, files) => {
    ...
    fs.writeFileSync(
        path.join(__dirname, "modelo_3.json"),
        JSON.stringify(resultado, null, 4),
        "utf-8"
    );

    console.log("\n✅ Resultado salvo em modelo_3.json");
});
